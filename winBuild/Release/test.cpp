...
void cDvbStream::fileSource (bool launchThread, const string& fileName) {
    ...
    if (launchThread)
        thread ([=]() {
        fileSourceInternal (true, fileName);
    } ).detach();// ,this
    else
    ...
    if (pmt->table_id == TID_PMT) {
        ...
        if (!getService (sid)) {
            ...
            while (streamLength > 0) {
                ...
                case  27: // H264 video
                    service.getStream (eVid).setPidTypeId (esPid, esPidInfo.getStreamTypeId());
                    break;
                ...
                case 129: // AC3 audio
                    service.setAudStream (esPid, esPidInfo.getStreamTypeId());
                    break;

                case   6: // subtitle
                    service.getStream (eSub).setPidTypeId (esPid, esPidInfo.getStreamTypeId());
                    break;
...
// return sectionLength
    ...
    case PID_PAT:
        parsePat (pidInfo, buf);
        break;
    case PID_NIT:
        parseNit (pidInfo, buf);
        break;
    case PID_SDT:
        parseSdt (pidInfo, buf);
        break;
    case PID_EIT:
        parseEit (pidInfo, buf);
        break;
    case PID_TDT:
        parseTdt (pidInfo, buf);
        break;
